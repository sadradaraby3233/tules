...
class SurgicalEditor:
    ...
    def _normalize_whitespace(self, text: str) -> str:
        ...
        normalized = [line for line in lines if line or not (normalized and not normalized[-1])]
        ...
        if full_path.suffix == '.py':
            ...
            if not is_valid:
                return CommandResult(
                    success=False,
                    message="SYNTAX_ERROR_PREVENTED: Your replacement would break the file. Save aborted.",
                    errors=[msg],
                    details={'hint': 'AI: Fix the syntax in your "replace_with" block and try again. Keep edits small (3-10 lines).'}
                )
    ...
    def surgical_replace(self, filepath: str, search: str, replace: str, reason: str = "Surgical edit") -> CommandResult:
        ...
        try:
            ...
            return CommandResult(success=False, message=f"Cannot read file: {e}")
        ...
        # Stage 2: Flexible Whitespace Regex Match (Ignores indentation hallucinations)
        import re
        ...
        if len(tokens) > 0:
            ...
            if len(matches) == 1:
                ...
                return CommandResult(success=False, message=f"AMBIGUOUS: Found {len(matches)} matches. Provide 2-3 more lines of context in 'search'.", details={'first_match_line': line_num})

        # Stage 3: AST-Based Match (Python only, for whole function/class replacement)
        if full_path.suffix == '.py':
            try:
        ...
        return CommandResult(success=False, message="SEARCH_FAILED: Could not find the 'search' block. Use 'read_file' to get exact text, or use 'replace_by_line'.", details={'tried': ['exact_match', 'flexible_regex', 'ast_match']})
    ...
    def replace_by_line(self, filepath: str, line_start: int, line_end: int, replace: str, reason: str = "Line-based replace") -> CommandResult:
        ...
        try:
            with open(full_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            # Adjust for 1-based indexing and ensure bounds
            ...
            return CommandResult(success=False, message=f"Line replace failed: {str(e)}")
...
class TulesAgent:
    ...
    def execute_single_command(self, cmd: Dict) -> CommandResult:
        ...
        try:
            ...
            elif action == 'surgical_replace':
                return self._handle_surgical_replace(cmd)
            elif action == 'replace_by_line':
                return self._handle_replace_by_line(cmd)
            elif action == 'smart_replace':
                return self._handle_smart_replace(cmd)
            elif action == 'confirm_smart_replace':
                return self._handle_confirm_smart_replace(cmd)
            elif action == 'extract_symbols':
                return self._handle_extract_symbols(cmd)
            elif action == 'memory':
                return self._handle_memory(cmd)
            elif action == 'help':
    ...
    def _handle_read_file(self, cmd: Dict) -> CommandResult:
        ...
        try:
            ...
            start_line = cmd.get('start_line')
            end_line = cmd.get('end_line')
            
            if start_line is not None and end_line is not None:
                start_idx = max(0, int(start_line) - 1)
                end_idx = min(len(lines), int(end_line))
                content = ''.join(lines[start_idx:end_idx])
                message = f"Read lines {start_line} to {end_line} of {filepath}"
            else:
                content = ''.join(lines)
    ...
    def _handle_replace(self, cmd: Dict) -> CommandResult:
        ...
        result = self.editor.execute_edit(op)
        if result.success and cmd.get('auto_verify'):
            verify = self._verify_file(op.file_path)
            if not verify.success:
                shutil.copy2(result.details['backup_created'], self.root_dir / op.file_path)
                return verify
        return result
    ...
    def _handle_smart_replace(self, cmd: Dict) -> CommandResult:
        ...
        if not filepath or not search_str:
            return CommandResult(success=False, message="Missing 'file' or 'search'")
        full_path = self.root_dir / filepath
        if not full_path.exists():
            return CommandResult(success=False, message=f"File not found: {filepath}")
        try:
            ...
            while True:
                ...
                matches.append({'id': len(matches), 'line': line_num, 'context': context})
            ...
            if len(matches) == 1:
                ...
                with open(full_path, 'w', encoding='utf-8') as f:
                    f.write(new_content)
                if cmd.get('auto_verify'):
                    verify = self._verify_file(filepath)
                    if not verify.success:
                        shutil.copy2(backup_path, full_path)
                        return verify
                return CommandResult(success=True, message=f"Auto-replaced 1 match in {filepath}", details={'backup': backup_path})
            return CommandResult(success=False, message=f"AMBIGUOUS: Found {len(matches)} matches. Use 'confirm_smart_replace' with the 'match_id'.", details={'matches': matches})
        except Exception as e:
    ...
    def _handle_confirm_smart_replace(self, cmd: Dict) -> CommandResult:
        ...
        try:
            ...
            for i in range(match_id + 1):
                idx = content.find(search_str, start)
                if idx == -1:
                    return CommandResult(success=False, message=f"Match ID {match_id} not found.")
                if i == match_id:
                    target_idx = idx
                    break
                start = idx + len(search_str)
            ...
            if cmd.get('auto_verify'):
                ...
                if not verify.success:
                    shutil.copy2(backup_path, full_path)
                    return verify
            return CommandResult(success=True, message=f"Successfully replaced match ID {match_id} in {filepath}", details={'backup': backup_path})
        except Exception as e:
            return CommandResult(success=False, message=f"Failed: {str(e)}")
    ...
    def _handle_extract_symbols(self, cmd: Dict) -> CommandResult:
        filepath = cmd.get('file', '')
        if not filepath:
            return CommandResult(success=False, message="Missing 'file'")
        full_path = self.root_dir / filepath
        if not full_path.exists():
            return CommandResult(success=False, message=f"File not found: {filepath}")
        ...
        try:
            ...
            return CommandResult(success=True, message=f"Extracted {len(symbols)} symbols from {filepath}", details={'symbols': symbols})
        except Exception as e:
            return CommandResult(success=False, message=f"Failed to extract symbols: {str(e)}")

    def _handle_memory(self, cmd: Dict) -> CommandResult:
        action_type = cmd.get('action_type', '')
        content = cmd.get('content', '')
        target = cmd.get('target', 'scratchpad')
        tules_dir = self.root_dir / '.tules'
        tules_dir.mkdir(exist_ok=True)
        filename = 'scratchpad.md' if target == 'scratchpad' else 'todo.md'
        filepath = tules_dir / filename
        try:
            if action_type == 'write':
                ...
                if not filepath.exists():
                    return CommandResult(success=True, message=f"{filename} is empty/does not exist yet.", details={'content': ''})
                with open(filepath, 'r', encoding='utf-8') as f:
                    return CommandResult(success=True, message=f"Read {filename}", details={'content': f.read()})
            else:
                return CommandResult(success=False, message="action_type must be 'write' or 'read'")
        except Exception as e:
            return CommandResult(success=False, message=f"Memory operation failed: {str(e)}")

    def _handle_help(self, cmd: Dict) -> CommandResult:
        help_text = """Available actions:
- read_file: Read file content (params: file, start_line, end_line)
- search: Exact text search (params: search, case_sensitive, whole_word)
- search_regex: Regex search (params: search)
- search_fuzzy: Fuzzy search (params: search, threshold)
- replace: Replace text in file (params: file, search, replace_with, reason, auto_verify)
- insert: Insert text at line (params: file, content, line_start, reason)
- delete: Delete lines (params: file, line_start, line_end, reason)
- analyze: Analyze file or project structure (params: file)
- list_files: List files matching pattern (params: pattern)
- run: Execute shell command (params: command)
- create_file: Create new file (params: file, content, auto_verify)
- delete_file: Delete file (params: file)
- search_and_replace_all: Replace all occurrences (params: file, search, replace_with)
- smart_replace: Smart replace with context matching (params: file, search, replace_with, auto_verify)
- confirm_smart_replace: Confirm specific match ID (params: file, search, replace_with, match_id, auto_verify)
- extract_symbols: Extract functions/classes (params: file)
- memory: Read/write to scratchpad or todo (params: action_type, content, target)
- help: Show this help message
"""
        return CommandResult(success=True, message="Available actions", details={'help': help_text})

    def _verify_file(self, filepath: str) -> CommandResult:
        full_path = self.root_dir / filepath
        if full_path.suffix == '.py':
            try:
                with open(full_path, 'r', encoding='utf-8') as f:
                ...
                return CommandResult(success=False, message=f"AUTO_VERIFY_FAILED: Syntax error in {filepath}. Rollback initiated.", errors=[f"Line {e.lineno}, Offset {e.offset}: {e.msg}"])
        return CommandResult(success=True, message="Verification passed")
    ...
    def _handle_surgical_replace(self, cmd: Dict) -> CommandResult:
        return self.editor.surgical_replace(
            filepath=cmd.get('file', ''),
            search=cmd.get('search', ''),
            replace=cmd.get('replace_with', ''),
            reason=cmd.get('reason', 'Surgical AI edit')
        )

    def _handle_replace_by_line(self, cmd: Dict) -> CommandResult:
        return self.editor.replace_by_line(
            filepath=cmd.get('file', ''),
            line_start=int(cmd.get('line_start', 1)),
            line_end=int(cmd.get('line_end', 1)),
            replace=cmd.get('replace_with', ''),
            reason=cmd.get('reason', 'Line-based AI edit')
        )
...
class ClipboardMonitor:
    ...
    def process_clipboard(self, text: str):
        # Matches optional markdown code blocks (e.g., ```json), 'edit:', payload, 'endedit', and optional closing code block
        pattern = r'(?is)(?:```[a-zA-Z]*\s*)?edit:\s*(.*?)\s*endedit(?:\s*```)?'
        match = re.search(pattern, text)
        if match:
            json_payload = match.group(1).strip()
            # Strip any stray triple quotes that AI models might add
    ...
    def execute_command(self, json_payload: str):
        try:
            ...
            # Fallback: Try to extract JSON from wrapped payload (e.g., if AI added extra text or quotes)
            start_idx_obj = json_payload.find('{')
            start_idx_arr = json_payload.find('[')
            starts = [i for i in (start_idx_obj, start_idx_arr) if i != -1]
            start_idx = min(starts) if starts else -1
            
            if start_idx != -1:
                end_idx_obj = json_payload.rfind('}')
                end_idx_arr = json_payload.rfind(']')
                ends = [i for i in (end_idx_obj, end_idx_arr) if i != -1]
                end_idx = max(ends) if ends else -1
                
                if end_idx > start_idx:
                    cleaned_payload = json_payload[start_idx:end_idx+1]
                    try:
                        parsed_data = json.loads(cleaned_payload)
                        print("[*] Successfully extracted JSON from wrapped payload.")
                    except json.JSONDecodeError:
                        raise ValueError("Could not parse JSON even after cleaning.")
                else:
                    raise ValueError("No JSON object or array found in payload.")
            else:
                raise ValueError("Invalid JSON in command.")
