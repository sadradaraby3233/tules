...
class SmartSearcher:
    def __init__(self, root_dir: str = "."):
        ...
        self.exclude_dirs = {
            ...
            'target', 'bin', 'obj', '.DS_Store', '.tules_backups'
        }
...
class SurgicalEditor:
    ...
    def _validate_and_save(self, full_path: Path, original: str, new_content: str, filepath: str, reason: str) -> CommandResult:
        if full_path.suffix == '.py':
            ...
            if not is_valid:
                return CommandResult(success=False, message="SYNTAX_ERROR_PREVENTED: Your replacement would break the file. Save aborted.", errors=[msg], details={'hint': 'Fix the syntax in your "replace_with" block and try again.'})
    ...
    def validate_edit(self, operation: EditOperation) -> Tuple[bool, List[str]]:
        ...
        if operation.operation_type == 'replace' and operation.old_content and operation.old_content not in current_content:
            errors.append("Old content not found in file.")
            return False, errors
    ...
    def execute_edit(self, operation: EditOperation) -> CommandResult:
        ...
        try:
            ...
            # Pre-flight syntax check for legacy edits too!
            if filepath.suffix == '.py':
                is_valid, msg = self._validate_python_syntax(new_content)
                if not is_valid:
                    return CommandResult(success=False, message="SYNTAX_ERROR_PREVENTED", errors=[msg])
...
class ClipboardMonitor:
    ...
    def _extract_json_robust(self, text: str) -> Optional[str]:
        """Depth-tracking JSON extractor. Never gets confused by nested braces."""
        start = -1
